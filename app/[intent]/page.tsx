...
export default function IntentPage({ params }: { params: { intent: string } }) {
  ...
  return (
    <article className="py-10">
      ...
      <div className="prose-tarot mt-6 leading-relaxed text-cream-dim">
        {page.intro.map((p, i) => (
          <p key={i}>{p}</p>
        ))}
      </div>
      ...
      <section className="mt-10">
        ...
        <div className="prose-tarot mt-4 leading-relaxed text-cream-dim">
      ...
      <section className="mt-10">
        ...
        <dl className="mt-4 space-y-5">
          {page.faq.map((f) => (
            <div key={f.q}>
              <dt className="font-medium text-cream">{f.q}</dt>
              <dd className="mt-1 text-sm leading-relaxed text-cream-dim">{f.a}</dd>
            </div>
          ))}
        </dl>
      </section>
