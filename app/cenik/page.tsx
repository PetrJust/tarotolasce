...
export default function CenikPage() {
  return (
    <div className="py-10">
      ...
      />
      <h1 className="font-display text-4xl font-semibold text-cream">Ceník</h1>
      ...
      <div className="mt-10 grid gap-5 sm:grid-cols-3">
        {TIERS.map((t) => (
          <div
            key={t.name}
            className="flex flex-col rounded-2xl border border-night-line bg-night-soft/60 p-6"
          >
            <h2 className="font-display text-2xl font-semibold text-cream">
              {t.name}
            </h2>
            <p className="mt-2 font-display text-3xl text-gold-soft">{t.price}</p>
            <p className="mt-1 text-xs text-cream-dim">{t.note}</p>
            <p className="mt-4 flex-1 text-sm text-cream-dim">{t.detail}</p>
