      ...
      <div className={phase === "picking" ? "flex-none" : "flex-1"}>
        {phase === "intro" && (
          <div className="flex h-full flex-col items-center justify-center gap-8 py-16 text-center">
            {/* F.6 intro copy PŘESNĚ dle zadání v1.1 */}
            <p className="mx-auto max-w-md font-display text-3xl font-semibold leading-snug text-body">
              Na chvíli se zastav. Zůstaň u své otázky. Až budeš připravená,{" "}
              {PERSONA_NAME} zamíchá karty.
            </p>
            <button
              onClick={() => doShuffle(true)}
              className="rounded-xl bg-love px-8 py-4 text-lg font-medium text-plum-900 shadow-glow hover:opacity-90"
            >
              Zamíchat karty
            </button>
        ...
        {phase === "shuffling" && (
          <div className="flex h-full flex-col items-center justify-center gap-6 py-20">
            ...
            <p className="text-body-dim">{PERSONA_NAME} míchá tvoje karty…</p>
          </div>
        )}
